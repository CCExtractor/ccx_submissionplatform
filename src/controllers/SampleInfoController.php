    ...
    function register(App $app)
    {
        ...
        $app->group('/sample-info', function () use ($self) {
            ...
            $this->get('/{id:[0-9]+}', function ($request, $response, $args) use ($self) {
                ...
                if($sample !== false){
                    ...
                    if($media !== false){
                        ...
                        $this->templateValues->add("additional_files", $dba->getAdditionalFiles($sample));
                        // Render
            ...
            $this->get('/{hash:[a-z0-9]+}', function ($request, $response, $args) use ($self) {
                ...
                // Fetch sample
                /** @var SampleData $sample */
                $sample = $dba->getSampleByHash($args["hash"]);
                if($sample !== false){
                    // Fetch media info
                    $media = $this->file_handler->fetchMediaInfo($sample);
                    if($media !== false){
            ...
            $this->group('/download/{id:[0-9]+}', function() use ($self){
                ...
                $this->get('/additional/{additional:[0-9]+}', function ($request, $response, $args) use ($self) {
                    ...
                    if($sample !== false){
                        ...
                        $extra = $dba->getAdditionalFile($sample,$args["additional"]);
                        if($extra !== false){
